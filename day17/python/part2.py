...
class Cube:
    def __init__(self, lines: List[str]) -> None:
        self.d: Dict[Point, bool] = {}    # key: point in 4D, value: active (true) / inactive (false)
        for y, line in enumerate(lines):
            for x, c in enumerate(line):
                self.d[Point(x, y, 0, 0)] = (c == '#')
            #
        #
    ...
    def get_neighbors(self, center: Point) -> List[Tuple[Point, bool]]:
        ...
        for elem in itertools.product([x-1, x, x+1], [y-1, y, y+1], [z-1, z, z+1], [w-1, w, w+1]):
            if elem != (x, y, z, w):
                p = Point(x=elem[0], y=elem[1], z=elem[2], w=elem[3])
                if self.d.get(p):
    ...
    def evolve(self) -> None:
        ...
        for point, status in self.d.items():
            ...
            assert len(neighbors) == 80    # (3 * 3 * 3 * 3) - 1
